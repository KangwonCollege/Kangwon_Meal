...
from app.models.dormitory_response import DormitoryResponse
from app.models.enumeration.dormitory_meal_type import DomitoryMealType
from app.utils.weekday import Weekday


class DormitoryMeal(BaseMeal):
    def __init__(self):
        super().__init__("https://knudorm.kangwon.ac.kr")
    ...
    async def _fetch_meal(
        ...
        for dormitory_type in DomitoryMealType:
            ...
            for j, meal_info in enumerate(meal[1:]):
                ...
                for k, meal_type in enumerate(['breakfast', 'lunch', 'dinner']):
                    ...
                    meal_info_day_and_type = meal_info_day.text.replace('\t', '').strip('\n')
